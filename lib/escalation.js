...
class Escalation extends events.EventEmitter {
	...
	constructor( backends, name, hormone, levels ) {
		...
		this.emit( 'escalate', this._id, this._name );
	...
	_escalate( level ) {
		...
		this._escalationTimeout = setTimeout( () => {
			...
			this.emit( 'escalatedLevel', this._id, this._name, level );
			...
			curLevel.recipients.forEach( ( recipient ) => {
				...
				try {
					...
					this.emit( 'msgEscalation', this._id, this._name, level, recipient );
	...
	_acknowledge( user ) {
		...
		this._recipients.forEach( ( recipient ) => {
			...
			try {
				...
				this.emit( 'msgAcknowledged', this._id, this._name, user, recipient );
		...
		this.emit( 'acknowledged', this._id, this._name, user );
	...
	deescalate() {
		...
		this._recipients.forEach( ( recipient ) => {
			try {
				...
				this.emit( 'msgDeescalate', this._id, this._name, recipient );
		...
		this.emit( 'deescalated', this._id, this._name );

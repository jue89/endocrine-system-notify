...
class Notify extends events.EventEmitter {
	...
	constructor( options ) {
		...
		if( options.escalateHormoneExpiration ) {
			...
			this._receptorListeners.hormoneExpiration = ( name, hormone ) => this._escalate( name, hormone );
			options.receptor.on(
				'hormoneExpiration',
			...
			this._receptorListeners.hormoneRefresh = ( name, hormone ) => this._escalate( name, hormone );
			options.receptor.on(
				'hormoneRefresh',
		...
		if( options.escalateHormoneError ) {
			...
			// Escalate on error
			this._receptorListeners.hormoneError = ( name, hormone ) => this._escalate( name, hormone );
			options.receptor.on(
				'hormoneError',
			...
			this._receptorListeners.hormoneRecovery = ( name, hormone ) => this._deescalate( name, hormone );
			options.receptor.on(
				'hormoneRecovery',
		...
		this._receptorListeners.undefined = ( name ) => this._deescalate( name );
		options.receptor.on(
			'undefined',
		...
		if( this._escalations[ name ] ) {
			...
			eLevelsDeferred.then( ( levels ) => {
				...
				try {
					...
					// Bypass error and acknowledge events
					escalation.on( 'error', ( e ) => this.emit( 'error', e ) );
					escalation.on( 'acknowledged', ( name, user ) => this.emit( 'acknowledged', name, user ) );
	...
	_deescalate( name, hormone ) {
		...
		if(
			...
			// Remove error and acknowledge listener
			this._escalations[ name ].removeAllListeners( 'error' );
			this._escalations[ name ].removeAllListeners( 'acknowledged' );
